  ...
  return (
    ...
    >
      ...
      <Box id="reviews">
        <Heading my="5vh">
          Reviews{' '}
          {commentsTotal.length >= 50 ? `(${commentsTotal.length})` : ''}
        </Heading>

        <Reviews
          comments={comments}
          totalComments={commentsTotal.length}
          starRating={commentsTotal.length}
        ></Reviews>
        ...
        <Box maxW="container.md" m="0 auto">
          ...
          <Box textAlign={'center'}>
            <Button
              mt="50px"
              variant={reviewButton ? 'link' : 'outline'}
              onClick={() => setReviewButton(current => !current)}
            >
              {reviewButton ? 'Close review form' : 'Write a review'}
            </Button>
          </Box>
        </Box>
      </Box>
...
export async function getServerSideProps() {  
  ...
  const postsTotal = firestore.collection('comments').orderBy('time', 'desc')
