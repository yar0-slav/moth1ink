          ...
          <form onSubmit={addComment}>
            <Box display={submitted ? 'none' : 'block'}>
              <StarsRating
                allowHalf={false}
                symbol={
                  <Icon as={IoStar} fontSize={{ base: '50px', md: '85px' }} />
                }
                ...
                {values.map((outerArray, columnIndex) => {
                  return (
                    <Box flexBasis={'49%'} key={columnIndex}>
                      <FormLabel>
                        {' '}
                        {columnIndex === 0 ? 'Pros' : 'Cons'}{' '}
                      </FormLabel>
                      {outerArray.inputValues.map(
                        (innerArray, innerObjectIndex) => {
                          return (
                            <InputGroup
                              display={'flex'}
            ...
            {submitted ? (
              <Text>
                Your comment was submited, thanks for taking the time!
              </Text>
            ) : (
              ''
            )}
